...
class FieldMapper
{
    ...
    public function map(array $row): NormalizedRecord
    {
        ...
        $serviceDate = trim((string) ($row['service_date'] ?? '')) ?: null;
        ...
        return new NormalizedRecord(
            ...
            customerName: $this->maskCustomerName($customerId ?? $this->stringOrNull($row['id'] ?? null) ?? '0'),
            ...
            status: $this->mapStatus($row, $completed, $cancelled),
            ...
            scheduledDateRaw: $serviceDate,
        );
    }

    // convert Launch27 status flags to our internal status
    private function mapStatus(array $row, bool $completed, bool $cancelled): string
    {
        if ($cancelled) {
            return 'cancelled';
        }
        ...
        return strtolower(trim((string) ($row['booking_status'] ?? 'unknown')));
    }
    ...
    // demo masking — don't store real customer names
